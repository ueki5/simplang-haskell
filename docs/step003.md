...
## 3. 比較・否定の型チェック（`expected` の一様伝播が破綻する箇所）
...
- bool変数のスタック格納幅は現状 `i32` 用の32bitスロットを流用しており、1バイトへのタイトパッキングは行っていない（実装の単純さを優先した意図的な判断）
- 比較演算子のチェイン（`a == b == c` のような書き方）は文法上は自然に許容されるが、意図的にサポートを謳っているわけではない（`(a == b)` の結果がbool型なので、`bool == c` として型チェック上は問題なく通る）
