...
- ポインタ型（`&`/`*`）の設計: `docs/step012.md`
- 数値リテラルの型サフィックス（i32/i64）の設計: `docs/step013.md`
- 変数宣言の型注釈省略と型推論の設計: `docs/step014.md`
- 関数の仮引数・戻り値型注釈省略と型推論の設計: `docs/step015.md`
...
- **型システム**: `Width`（`W32`/`W64`）がソース型名（`i32`/`i64`）とcodegenのレジスタ幅を兼用する唯一の型。`Type`（`TyInt Width | TBool | TPtr Type`）はそれをさらにbool・ポインタ（再帰的、`&`で任意段数ネスト可能）まで拡張したソースレベルの論理型で、ポインタは指す先の型によらず常に`W64`（8バイト）で格納される。`compileExprTyped` が期待型を式木に一様伝播させ、`Var` の実際の型と食い違えば `type mismatch` エラー。同一式内でのi32/i64混在は必ずこのチェックで検出される（式ノードへの型タグ付けやボトムアップ型推論は不要。唯一の例外が`9999i64`/`9999i32`のような型サフィックス付き整数リテラル`LitTyped Int Width`で、`Var`と同様に常に確定型を持つリーフとして`expected`との一致を厳密にチェックする）。`&式`（アドレス取得）の対象は構文的には任意の式を受け付けるが、`addressOf` ヘルパーが変数参照とポインタのデリファレンス（lvalue）のみに意味論的に制限する。`let`の型注釈は省略可能（`SLetInferred`）で、その場合は暗黙main末尾式の型決定に使われているのと同じ `inferType`（＝`inferMaybeType`が`Nothing`なら`i64`をデフォルトとする）で型を決定してから`SLet`と同じ処理に合流する
- **fnの仮引数・戻り値型の省略と推論**: `FnDecl`のパラメータ型・戻り値型フィールドは`Maybe Type`（`fn add(a, b) { a + b }`のように独立に省略可能）。`compile`の1パス目は`resolveFnSigs`（`buildFnSigs`を置き換え）が担い、`main`予約名・重複定義・最大6引数の構造検証をまず行った上で、省略された型注釈を`Slot`（`ParamSlot 名前 引数番号 | ReturnSlot 名前`）として列挙し、固定点ループで解決する：プログラム全体（全fn本体＋暗黙main）を1回走査するたびに、パラメータ型は呼び出し箇所の実引数式から、戻り値型は自身の`return`文・末尾式から証拠を集め（`programEvidence`/`resolveExprType`＝`inferMaybeType`のLocalEnv版）、`unifyMaybeType`で単一化できたスロットから確定させ、証拠が無ければ`i64`にデフォルトする（`inferType`と同じ規則）。あるスロットの証拠が「自分自身待ち」であれば依存関係として数えず無視する（`fact`のような通常の自己再帰・`is_even`/`is_odd`のような相互再帰を循環と誤検出しないための要）。1ラウンドで1つも前進しなければ、それは（自己ループを除く）真の循環依存であり`"circular type inference: ..."`エラーとする。`FnSigs`が確定した後段（`compileFnDecl`以降）は無変更で、`compileFnDecl`のみ`FnDecl`自身ではなく確定済み`fnSigs`から具体的な型を取得する
...
- **出力**: コンパイルされたバイナリは `printf` で結果を標準出力へ表示（末尾式の型が `i64` なら `%ld`、`i32` なら `%d`、ポインタ型なら `%p`）
